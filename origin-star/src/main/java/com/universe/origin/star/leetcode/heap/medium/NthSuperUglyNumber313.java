...
public class NthSuperUglyNumber313 {
    public static void main(String[] args) {
        ...
        nthSuperUglyNumber313.nthSuperUglyNumber(12,new int[]{2,7,13,19});
    }
    ...
    public int nthSuperUglyNumber(int n, int[] primes) {
        ...
        for (int i = 1; i < n; i++) {
            // 找到 result[index] * primes 的最小值
            int currentMin = result[index[0]] * primes[0];
            List<Integer> minIndex = new ArrayList<>();
            minIndex.add(0);
            for (int j = 1; j < primes.length; j++) {
                int tempNum = result[index[j]] * primes[j];
                if (tempNum < currentMin){
                    currentMin = tempNum;
                    minIndex.clear();
                    minIndex.add(j);
                }else if (tempNum == currentMin){
                    minIndex.add(j);
                }
            }
            //游标前移
            for (int j = 0; j < minIndex.size(); j++) {
                index[minIndex.get(j)] +=1;
            }
            result[i] = currentMin;
        }
        return result[n-1];
